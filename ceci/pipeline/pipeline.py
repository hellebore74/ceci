...
class Pipeline:
    ...
    def build_config(
        ...
        template_parameters=None, yamlId=None
    ):
        ...
        # Then parse with YAML
        #   if the file contains only one configuraton, convert the yaml input into configuration
        if not yamlId:
            pipe_config = yaml.safe_load(config_text)
        else:
            import copy
            ...
            print("LAPP - read yaml config file : ",yamlId)

            # Looping over yaml file to find yamlid section 
            for config in pipe_config_gen:
                if config and "id" in config and config["id"]==yamlId: 
                    pipe_config = copy.deepcopy(config)
            if not pipe_config:
                raise ValueError(f"LAPP - Could not find yaml section with id value set to {yamlId}")

            print("LAPP - final yaml config : ", pipe_config)
